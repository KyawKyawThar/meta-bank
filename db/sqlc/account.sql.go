...
const getAccountForUpdateTransaction = `-- name: GetAccountForUpdateTransaction :one
SELECT id, owner, currency, balance, created_at
FROM accounts
WHERE id = $1 LIMIT 1 FOR NO KEY UPDATE
`

func (q *Queries) GetAccountForUpdateTransaction(ctx context.Context, id int64) (Account, error) {
	row := q.db.QueryRow(ctx, getAccountForUpdateTransaction, id)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
